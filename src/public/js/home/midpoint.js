...
function isContained(places, y, x) {
    ...
    for (var i = 0; i < places.length; i++) {
        ...
        if (midBounds.contain(coord)) {
            ...
            rec_name.push(places[i].place_name);
            rec_addr.push(places[i].addr);
        }
    }

    if(rec_name.length > 0){
      console.log(rec_name);
      console.log(rec_addr);
      for (var i = 0; i < rec_name.length; i++) {
        searchPlaces(rec_name[i]);
      }
    }
...
function searchPlaces(name) {
  ps.keywordSearch(name, keywordSearchCB);
}
...
function keywordSearchCB(data, status, pagination) {
    if (status === kakao.maps.services.Status.OK) {
        ...
        for(var i = 0; i < data.length; i++){
          if(data[i].road_address_name == rec_addr[0]){
